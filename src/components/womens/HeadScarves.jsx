...
import styles from "../../styles/womens/headwares.module.css"
...
const HeadScarves = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch(
        `https://evening-scrubland-37768.herokuapp.com/womens/headwares/?type=${type}&size=${size}&colors=${colors}&styles=${isStyle}`
      )
  ...
  useEffect(() => {
    async function getPopularProducts() {
      const response = await fetch(
        "https://evening-scrubland-37768.herokuapp.com/womens/headwares/offers"
      )
      const data = await response.json()
  ...
  return (
    <section className={styles.main}>
      <h1 className={styles.heading}>Headwares and Footwares Collections</h1>
          ...
          <section className={styles.filters}>
            ...
            {isTypeClicked && (
              <section className={styles.typeFilter}>
                ...
                <label className={styles.label} htmlFor="sandals">
                  Sandals{" "}
                  <input
                    type="checkbox"
                    value="sandals"
                ...
                <label className={styles.label} htmlFor="stole">
                  Stole{" "}
                  <input
                    type="checkbox"
                    value="stole"
                ...
                <label className={styles.label} htmlFor="scarf">
                  Scarf{" "}
                  <input
                    type="checkbox"
                    value="scarf"
          ...
          <section className={styles.filters}>
            ...
            {isStylesClicked && (
              <section className={styles.typeFilter}>
                ...
                <label className={styles.label} htmlFor="heels">
                  Heels{" "}
                  <input
                    type="checkbox"
                    value="heels"
          ...
          {loading ? (
            ...
            currentPosts.map(product => (
              <section key={product._id}>
                <Card
                  title={product.name}
                  link={`/products/womens/headwares-scarves/${product._id}`}
                  src={product.Image}
                  price={product.price}
                />
              </section>
            ...
            popular.map(item => (
              <section key={item._id}>
                <Card
                  ...
                  title={item.name}
                  price={item.price}
                  link={`/products/womens/headwares-scarves/${item._id}`}
                />
